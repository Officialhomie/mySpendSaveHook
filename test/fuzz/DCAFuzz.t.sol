...
contract DCAFuzz is Test {
    ...
    function testFuzz_dcaDeadlineValidation(
        ...
        deadline = bound(deadline, currentTime - 30 days, currentTime + 365 days);
    ...
    function testFuzz_dcaTickCalculation(
        ...
        // Convert signed to unsigned for bound, then back to signed
        int256 offset = type(int256).max;
        uint256 tickUnsigned = uint256(tickInput + offset);
        uint256 minTickUnsigned = uint256(int256(minTick) + offset);
        uint256 maxTickUnsigned = uint256(int256(maxTick) + offset);
        uint256 boundedUnsigned = bound(tickUnsigned, minTickUnsigned, maxTickUnsigned);
        int24 tick = int24(int256(boundedUnsigned) - offset);
    ...
    function testFuzz_dcaBatchExecution(
        ...
        vm.assume(inputAmounts.length > 0 && inputAmounts.length <= 10);
        vm.assume(inputAmounts.length == outputAmounts.length);
        slippageBps = bound(slippageBps, 0, 10000);
