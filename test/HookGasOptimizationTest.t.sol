...
contract HookGasOptimizationTest is Test, Deployers {
    ...
    function testGasOptimization_OutputSavingsWithDCA() public {
        ...
        vm.prank(address(hook));
        (bytes4 afterSelector, int128 hookDelta) = hook._afterSwapInternal(
            bob, poolKey, params, swapDelta, hookData
        );
        uint256 afterSwapGas = gasBeforeAfter - gasleft();
    ...
    function testGasOptimization_NoSavingsScenario() public {
        ...
        vm.prank(address(hook));
        (bytes4 beforeSelector, BeforeSwapDelta beforeDelta, uint24 fee) = hook._beforeSwapInternal(
            noSavingsUser, poolKey, params, hookData
        );
        uint256 beforeSwapGas = gasBeforeBefore - gasleft();
        ...
        vm.prank(address(hook));
        (bytes4 afterSelector, int128 hookDelta) = hook._afterSwapInternal(
            noSavingsUser, poolKey, params, swapDelta, hookData
        );
        uint256 afterSwapGas = gasBeforeAfter - gasleft();
    ...
    function testGasOptimization_ExtremeSavingsScenario() public {
        ...
        strategyModule.setSavingStrategy(
            extremeUser, 5000, 0, 0, false, // 50% savings
            SpendSaveStorage.SavingsTokenType.OUTPUT, address(tokenB)
        );
        ...
        vm.prank(address(hook));
        (bytes4 beforeSelector, BeforeSwapDelta beforeDelta, uint24 fee) = hook._beforeSwapInternal(
            extremeUser, poolKey, params, hookData
        );
        uint256 beforeSwapGas = gasBeforeBefore - gasleft();
        ...
        vm.prank(address(hook));
        (bytes4 afterSelector, int128 hookDelta) = hook._afterSwapInternal(
            extremeUser, poolKey, params, swapDelta, hookData
        );
        uint256 afterSwapGas = gasBeforeAfter - gasleft();
