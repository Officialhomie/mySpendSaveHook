...
contract PackedStorageIntegrityTest is Test, Deployers {
    ...
    function _setupTestAccounts() internal {
        ...
        strategyModule.setSavingStrategy(
            charlie, 5000, 0, 0, false, 
            SpendSaveStorage.SavingsTokenType.SPECIFIC, address(tokenA)
        );
        vm.stopPrank();
    ...
    function testPackedStorage_PackedUserConfigIntegrity() public {
        ...
        (percentage, roundUp, savingsType, enableDCA) = 
    ...
    function testPackedStorage_SingleSlotOperations() public {
        ...
        vm.startPrank(alice);
        strategyModule.setSavingStrategy(
            alice, 1500, 0, 0, true, // Enable round up
            SpendSaveStorage.SavingsTokenType.OUTPUT, address(tokenB)
        );
        vm.stopPrank();
    ...
    function testPackedStorage_BitFieldBoundaries() public {
        ...
        for (uint8 i = 0; i < 3; i++) {
            strategyModule.setSavingStrategy(
                bob, 1000, 0, 0, false, 
                SpendSaveStorage.SavingsTokenType(i), address(tokenA)
            );
    ...
    function testPackedStorage_MemoryVsStorageConsistency() public {
        ...
        strategyModule.setSavingStrategy(
            alice, 2500, 0, 0, true, 
            SpendSaveStorage.SavingsTokenType.SPECIFIC, address(tokenB)
        );
        vm.stopPrank();
    ...
    function testPackedStorage_ConcurrentAccessSafety() public {
        ...
        vm.startPrank(user1);
        strategyModule.setSavingStrategy(user1, 1000, 0, 0, false, 
            SpendSaveStorage.SavingsTokenType.INPUT, address(tokenA));
        vm.stopPrank();
        
        vm.startPrank(user2);
        strategyModule.setSavingStrategy(user2, 2000, 0, 0, true, 
            SpendSaveStorage.SavingsTokenType.OUTPUT, address(tokenB));
        vm.stopPrank();
        
        vm.startPrank(user3);
        strategyModule.setSavingStrategy(user3, 3000, 0, 0, false, 
            SpendSaveStorage.SavingsTokenType.SPECIFIC, address(tokenA));
        vm.stopPrank();
