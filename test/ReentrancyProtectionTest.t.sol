...
contract ReentrancyProtectionTest is Test, Deployers {
    ...
    function testReentrancyProtection_ProcessDailySavingsBlocked() public {
        ...
        vm.prank(alice);
        strategyModule.setSavingStrategy(
            alice, 1500, 0, 0, false, SpendSaveStorage.SavingsTokenType.INPUT, address(tokenA)
        );
    ...
    function testReentrancyProtection_HookFunctionsSafeFromReentrancy() public {
        ...
        vm.prank(address(hook));
        (bytes4 selector, int128 hookDelta) = hook._afterSwapInternal(alice, poolKey, params, swapDelta, hookData);
    ...
    function testReentrancyProtection_StateConsistencyUnderReentrancy() public {
        ...
        StateManipulatorContract manipulator = new StateManipulatorContract(address(storageContract), alice);
    ...
    function testReentrancyProtection_TransientStorageIntegrity() public {
        ...
        vm.prank(address(hook));
        storageContract.setTransientSwapContext(alice, 0.2 ether, 2000, true, 0, false, false);

        // Deploy contract that attempts to interfere with transient storage
        TransientStorageAttacker attacker_contract = new TransientStorageAttacker(address(storageContract), alice);
    ...
    function testReentrancyProtection_CrossModuleSafety() public {
        ...
        CrossModuleAttacker crossAttacker = new CrossModuleAttacker(address(strategyModule), address(dcaModule), alice);
...
contract StateManipulatorContract {
    ...
    function attemptStateManipulation() external {
        ...
        storageContract.setTransientSwapContext(user, 999 ether, 9999, true, 0, false, false);
    }
}
...
contract CrossModuleAttacker {
    ...
    function attemptCrossModuleReentrancy() external {
        ...
        strategyModule.setSavingStrategy(user, 5000, 0, 0, false, SpendSaveStorage.SavingsTokenType.OUTPUT, address(0));
    }
}
