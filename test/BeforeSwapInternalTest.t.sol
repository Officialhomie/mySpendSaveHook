...
contract BeforeSwapInternalTest is Test, Deployers {
    ...
    function testBeforeSwapInternal_BasicFunctionality() public {
        ...
        assertEq(
            uint8(savingsTokenType), uint8(SpendSaveStorage.SavingsTokenType.INPUT), "Should have INPUT savings type"
        );
    ...
    function testBeforeSwapInternal_OutputSavingsType() public {
        ...
        assertEq(selector, IHooks.beforeSwap.selector, "Should return correct selector");
        assertEq(
            BeforeSwapDeltaLibrary.getSpecifiedDelta(delta), 0, "Should return zero specified delta for OUTPUT savings"
        );
        assertEq(
            BeforeSwapDeltaLibrary.getUnspecifiedDelta(delta),
            0,
            "Should return zero unspecified delta for OUTPUT savings"
        );
        assertEq(fee, 0, "Should not modify fee");
        ...
        assertEq(
            uint8(context.savingsTokenType),
            uint8(SpendSaveStorage.SavingsTokenType.OUTPUT),
            "Should set OUTPUT savings type"
        );
    ...
    function testBeforeSwapInternal_TransientStorageIntegrity() public {
        ...
        assertEq(
            uint8(aliceContext.savingsTokenType),
            uint8(SpendSaveStorage.SavingsTokenType.INPUT),
            "Should set savings type"
        );
        ...
        assertEq(
            uint8(bobContext.savingsTokenType),
            uint8(SpendSaveStorage.SavingsTokenType.OUTPUT),
            "Bob should have OUTPUT type"
        );
    ...
    function testBeforeSwapInternal_EdgeCases() public {
        ...
        SwapParams memory zeroParams = SwapParams({zeroForOne: true, amountSpecified: 0, sqrtPriceLimitX96: 0});

        vm.prank(address(hook));
        (bytes4 selector1, BeforeSwapDelta delta1, uint24 fee1) =
            hook._beforeSwapInternal(alice, poolKey, zeroParams, abi.encode(alice));

        assertEq(selector1, IHooks.beforeSwap.selector, "Should handle zero amount");
        assertEq(
            BeforeSwapDeltaLibrary.getSpecifiedDelta(delta1), 0, "Should return zero specified delta for zero amount"
        );
        assertEq(
            BeforeSwapDeltaLibrary.getUnspecifiedDelta(delta1),
            0,
            "Should return zero unspecified delta for zero amount"
        );
