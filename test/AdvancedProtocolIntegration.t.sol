...
contract AdvancedProtocolIntegration is Test, Deployers {
    ...
    function _setupUserAccounts() internal {
        ...
        vm.startPrank(bob);
        strategyModule.setSavingStrategy(
            bob, 500, 0, 10000, false, SpendSaveStorage.SavingsTokenType.OUTPUT, address(0)
        );
        vm.stopPrank();
    ...
    function testMulticallBatchOperations() public {
        ...
        calls[2] = abi.encodeWithSelector(bytes4(keccak256("getTreasuryFee()")));
    ...
    function _simulateSwapWithSavings(address user, address tokenIn, address tokenOut, uint256 amountIn) internal {
        // Get user savings strategy
        (uint256 percentage,, uint8 savingsTokenType,) = storageContract.getPackedUserConfig(user);
