...
contract BatchOperationsAtomicityTest is Test {
    ...
    function testBatchAtomicity_TokenMintSuccess() public {
        ...
        assertEq(
            tokenModule.balanceOf(alice, TOKEN_ID_A), initialBalanceA + amounts[0], "Token A balance should increase"
        );
        assertEq(
            tokenModule.balanceOf(alice, TOKEN_ID_B), initialBalanceB + amounts[1], "Token B balance should increase"
        );
        assertEq(
            tokenModule.balanceOf(alice, TOKEN_ID_C), initialBalanceC + amounts[2], "Token C balance should increase"
        );
    ...
    function testBatchAtomicity_TokenTransferSuccess() public {
        ...
        assertEq(
            tokenModule.balanceOf(bob, TOKEN_ID_A), bobBalanceA + amounts[0], "Bob Token A balance should increase"
        );
        assertEq(
            tokenModule.balanceOf(bob, TOKEN_ID_B), bobBalanceB + amounts[1], "Bob Token B balance should increase"
        );
    ...
    function testBatchAtomicity_TokenBurnSuccess() public {
        ...
        // Verify all operations succeeded atomically
        assertEq(
            tokenModule.balanceOf(alice, TOKEN_ID_A), initialBalanceA - amounts[0], "Token A balance should decrease"
        );
        assertEq(
            tokenModule.balanceOf(alice, TOKEN_ID_B), initialBalanceB - amounts[1], "Token B balance should decrease"
        );
