...
contract SpendSaveSlippageEnhanced {
     ...
     */
    function validateSwapSlippage(BalanceDelta delta, uint128 amountInMax, uint128 amountOutMin) external pure {
     ...
     */
    function calculateDynamicSlippage(address token, uint256 amount, uint256 baseSlippageBps)
        external
        view
    ...
    function _adjustForMarketConditions(uint256 baseSlippage) internal view returns (uint256) {
        ...
        if (hour >= 13 && hour <= 15) {
            // 1-3 PM UTC (market active hours)
    ...
    function _isVolatileToken(address token) internal pure returns (bool) {
        ...
        return !_isStableToken(token) && token != address(0x4200000000000000000000000000000000000006); // Not WETH
    }
}
