...
contract SlippageControl is ISlippageControlModule, ReentrancyGuard {
    ...
    function setSlippageTolerance(address user, uint256 basisPoints)
        external
        override
        onlyAuthorized(user)
        nonReentrant
    ...
    function setTokenSlippageTolerance(address user, address token, uint256 basisPoints)
        external
        override
        onlyAuthorized(user)
        nonReentrant
    ...
    function _emitSlippageExceeded(
        ...
        emit SlippageExceeded(user, fromToken, toToken, fromAmount, receivedAmount, expectedMinimum);
    }

    // Helper to apply slippage action
    function _applySlippageAction(address user, uint256 receivedAmount, uint256 expectedMinimum)
        internal
        view
        returns (bool)
    {
    ...
    function getEffectiveSlippageTolerance(address user, address token, uint256 customSlippageTolerance)
        internal
        view
        returns (uint256)
    {
