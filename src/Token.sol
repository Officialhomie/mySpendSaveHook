...
contract Token is ITokenModule, ReentrancyGuard {
    ...
    bytes4 private constant _ERC6909_RECEIVED = 0x05e3242b; // bytes4(keccak256("onERC6909Received(address,address,uint256,uint256,bytes)"))
    ...
    modifier onlyAuthorized(address user) {
        if (
            msg.sender != user && msg.sender != address(storage_) && msg.sender != storage_.spendSaveHook()
                && msg.sender != _savingsModule && msg.sender != _dailySavingsModule && !isOperator[user][msg.sender]
        ) {
     ...
     */
    function batchRegisterTokens(address[] calldata tokens) external override returns (uint256[] memory tokenIds) {
     ...
     */
    function batchMintSavingsTokens(address user, uint256[] calldata tokenIds, uint256[] calldata amounts)
        external
        override
        onlyAuthorized(user)
    {
     ...
     */
    function burnSavingsToken(address user, uint256 tokenId, uint256 amount)
        external
        override
        onlyAuthorized(user)
        nonReentrant
     ...
     */
    function batchBurnSavingsTokens(address user, uint256[] calldata tokenIds, uint256[] calldata amounts)
        external
        override
        onlyAuthorized(user)
        nonReentrant
     ...
     */
    function batchTransfer(address from, address to, uint256[] calldata tokenIds, uint256[] calldata amounts)
        external
        override
        onlyAuthorized(from)
        nonReentrant
    ...
    function transferFrom(address operator, address sender, address receiver, uint256 id, uint256 amount)
        external
        onlyAuthorized(operator)
        nonReentrant
        returns (bool)
    {
     ...
     */
    function approve(address spender, uint256 tokenId, uint256 amount) external override returns (bool success) {
        ...
        returns (bool success)
    {
        ...
        if (currentAllowance < amount) {
            revert InsufficientAllowance(from, msg.sender, tokenId, amount, currentAllowance);
        }
    ...
    function safeTransfer(address sender, address receiver, uint256 id, uint256 amount, bytes calldata data)
        external
        onlyAuthorized(sender)
        nonReentrant
        returns (bool)
