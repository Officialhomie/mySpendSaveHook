...
interface ISavingsModule is ISpendSaveModule {
    ...
    event SavingsProcessed(address indexed user, address indexed token, uint256 amount, uint256 netAmount, uint256 fee);

    event BatchSavingsProcessed(address indexed user, uint256 totalAmount, uint256 totalFee);
     ...
     */
    function getUserSavings(address user) external view returns (address[] memory tokens, uint256[] memory amounts);
     ...
     */
    function queueForDCA(address user, address fromToken, uint256 amount, address targetToken) external;
     ...
     */
    function emergencyWithdraw(address user, address token, uint256 amount, address recipient) external;
