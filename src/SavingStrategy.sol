...
contract SavingStrategy is ISavingStrategyModule, ReentrancyGuard {
    ...
    event SwapPrepared(
        address indexed user, uint256 currentSavePercentage, SpendSaveStorage.SavingsTokenType tokenType
    );
    ...
    event SavingsTransferSuccess(
        address indexed actualUser, address indexed token, uint256 amount, uint256 contractBalance
    );
    ...
    event InputTokenSaved(
        address indexed user, address indexed token, uint256 savedAmount, uint256 remainingSwapAmount
    );

    /// @notice Emitted when afterSwap processing is called
    event ProcessInputSavingsAfterSwapCalled(
        address indexed actualUser, address indexed inputToken, uint256 pendingSaveAmount
    );
    ...
    modifier onlyAuthorized(address user) {
        if (msg.sender != user && msg.sender != address(storage_) && msg.sender != storage_.spendSaveHook()) {
     ...
     */
    function setSavingsGoal(address user, address token, uint256 amount)
        external
        override
        onlyAuthorized(user)
        onlyInitialized
     ...
     */
    function beforeSwap(address actualUser, PoolKey calldata key, SwapParams calldata params)
        external
        override
        onlyHook
        onlyInitialized
     ...
     */
    function updateSavingStrategy(address user, SpendSaveStorage.SwapContext memory context)
        external
        override
        onlyHook
        onlyInitialized
     ...
     */
    function processInputSavingsAfterSwap(address actualUser, SpendSaveStorage.SwapContext memory context)
        external
        override
        onlyHook
        onlyInitialized
     ...
     */
    function calculateSavingsAmount(uint256 amount, uint256 percentage, bool roundUp)
        external
        pure
        override
        ...
        )
    {
        ...
        (uint256 packedPercentage, bool packedRoundUp, uint8 packedTokenType,) = storage_.getPackedUserConfig(user);
     ...
     */
    function previewSavings(address user, uint256 swapAmount)
        external
        view
        returns (uint256 saveAmount, uint256 remainingSwapAmount)
    {
