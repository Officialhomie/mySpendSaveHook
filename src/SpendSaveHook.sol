...
contract SpendSaveHook is BaseHook, ReentrancyGuard {
    ...
    event SpecificTokenSwapQueued(
        address indexed user, address indexed fromToken, address indexed toToken, uint256 amount
    );
     ...
     */
    constructor(IPoolManager _poolManager, SpendSaveStorage _storage) BaseHook(_poolManager) {
    ...
    function initializeModules(
        ...
        _initializeModuleReferences(_savingStrategy, _savings, _dca, _slippageControl, _token, _dailySavings);

        modulesInitialized = true;
        emit ModulesInitialized(_savingStrategy, _savings, _dca, _slippageControl, _token, _dailySavings);
    }
    ...
    function _initializeModuleReferences(
        ...
        IDCAModule(_dca).setModuleReferences(_savingStrategy, _savings, _dca, _slippageControl, _token, _dailySavings);
    ...
    function _beforeSwapInternal(
        ...
        storage_.setTransientSwapContext(
            user, uint128(saveAmount), uint128(percentage), true, savingsTokenType, roundUpSavings, enableDCA
        );
    ...
    function _afterSwap(
        ...
        try this._afterSwapInternal(sender, key, params, delta, hookData) returns (bytes4 selector, int128 hookDelta) {
     ...
     */
    function _calculateSavingsInMemory(uint256 amount, uint256 percentage, bool roundUp)
        internal
        pure
        returns (uint256 saveAmount)
    {
        ...
        assembly {
            ...
            if roundUp {
                ...
                if gt(remainder, 0) { saveAmount := add(saveAmount, 1) }
            }
        }
     ...
     */
    function _calculateBeforeSwapDelta(uint8 savingsTokenType, SwapParams calldata params, uint256 saveAmount)
        internal
        pure
    ...
    function _calculateOutputSavings(
        ...
        if (params.zeroForOne) {
            ...
            if (outputAmount > 0) {
                amount = _calculateSavingsInMemory(uint256(outputAmount), percentage, roundUp);
            }
        } else {
     ...
     */
    function _processBatch(address user, DailySavingsProcessor memory processor, uint256 startIdx, uint256 endIdx)
        internal
    {
     ...
     */
    function _processSingleToken(address user, address tokenAddr)
        external
        returns (uint256 savedAmount, bool success)
    {
    ...
    function checkModulesInitialized() external view returns (bool initialized) {
        return storage_.getModule(STRATEGY_MODULE) != address(0) && storage_.getModule(SAVINGS_MODULE) != address(0)
            && storage_.getModule(DCA_MODULE) != address(0) && storage_.getModule(SLIPPAGE_MODULE) != address(0)
            && storage_.getModule(TOKEN_MODULE) != address(0) && storage_.getModule(DAILY_MODULE) != address(0);
    }
