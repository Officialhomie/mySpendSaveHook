...
contract DailySavings is IDailySavingsModule {
    ...
    event DailySavingsYieldStrategySet(
        address indexed user, address indexed token, SpendSaveStorage.YieldStrategy strategy
    );
    ...
    function _validateConfigParams(address token, uint256 dailyAmount, uint256 penaltyBps, uint256 endTime)
        internal
        view
    {
    ...
    function _resetDailySavings(address user, address token) internal {
        SpendSaveStorage.DailySavingsConfigParams memory params = SpendSaveStorage.DailySavingsConfigParams({
            enabled: false,
    ...
    function _executeDailySavingsForTokenInternal(address user, address token) internal returns (uint256) {
        uint256 startGas = gasleft();
    ...
    function _prepareExecutionContext(address user, address token)
        internal
        view
        returns (ExecutionContext memory context)
    {
        ...
        if (!status.enabled) {
            return ExecutionContext({shouldProcess: false, amountToSave: 0, reason: "Savings not enabled"});
        }

        // Check if goal already reached
        if (status.goalReached) {
            return ExecutionContext({shouldProcess: false, amountToSave: 0, reason: "Goal already reached"});
        }
        ...
        if (amountToSave == 0) {
            return ExecutionContext({shouldProcess: false, amountToSave: 0, reason: "No amount to save"});
        }
        ...
        if (!sufficientFunds) {
            return ExecutionContext({shouldProcess: false, amountToSave: 0, reason: reason});
        }

        // All checks passed, ready to process
        return ExecutionContext({shouldProcess: true, amountToSave: amountToSave, reason: ""});
    }

    // Execute token savings with proper error handling
    function _executeTokenSavings(address user, address token, uint256 amount) internal returns (uint256) {
    ...
    function _updateSavingsState(address user, address token, uint256 amount) internal {
    ...
    function _checkGoalReached(address user, address token) internal {
        ...
        (,,, uint256 goalAmount, uint256 currentAmount,,) = storage_.getDailySavingsConfig(user, token);
    ...
    function _getDailySavingsStatus(address user, address token)
        internal
        view
        returns (DailySavingsStatus memory status)
    {
    ...
    function _calculateSavingsAmount(DailySavingsStatus memory status) internal pure returns (uint256) {
        ...
        if (status.goalAmount > 0) {
            uint256 remaining = status.goalAmount > status.currentAmount ? status.goalAmount - status.currentAmount : 0;
    ...
    function _checkAllowanceAndBalance(address user, address token, uint256 amount)
        internal
        view
        returns (bool sufficient, string memory reason)
    {
    ...
    function withdrawDailySavings(address user, address token, uint256 amount)
        external
        override
        onlyAuthorized(user)
        nonReentrant
    ...
    function _prepareWithdrawalContext(address user, address token, uint256 amount)
        internal
        view
    ...
    function _processWithdrawal(address user, address token, uint256 amount, WithdrawalContext memory context)
        internal
    {
    ...
    function getDailyExecutionStatus(address user, address token)
        public
        view
        override
        returns (bool canExecute, uint256 daysPassed, uint256 amountToSave)
    {
    ...
    function getDailySavingsStatus(address user, address token)
        external
        view
        override
        returns (
        ...
        )
    {
        ...
        remainingAmount = status.goalAmount > status.currentAmount ? status.goalAmount - status.currentAmount : 0;
        ...
        estimatedCompletionDate = _estimateCompletionDate(
            status.dailyAmount, remainingAmount, status.goalAmount, status.currentAmount, status.endTime
        );
     ...
     */
    function executeTokenSavings(address user, address token) external override returns (uint256 amount) {
        ...
        if (config.goalAmount > 0) {
            uint256 remaining = config.goalAmount > config.currentAmount ? config.goalAmount - config.currentAmount : 0;
            if (amount > remaining) {
                amount = remaining;
            }
        }
